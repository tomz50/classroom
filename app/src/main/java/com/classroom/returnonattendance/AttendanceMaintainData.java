...
public class AttendanceMaintainData extends AsyncTask<String, Void, String> { private ProgressDialog dialog ;
    Context c;
    String crlf = "\r\n";
    String twoHyphens = "--";
    String boundary =  "*****";
    ...
    public AttendanceMaintainData(Context context, String[] data) {
        this.c = context;
        getData = data;
    ...
    protected String doInBackground(String... strings) {
        ...
        try {
            ...
            request.writeBytes(crlf);
            request.writeBytes(twoHyphens + boundary + crlf);
            request.writeBytes("Content-Disposition: form-data; name=\"TrUserid\"" + "\"" + crlf);
            request.writeBytes(crlf);
            ...
            request.writeBytes(crlf);
            request.writeBytes(twoHyphens + boundary + crlf);
            request.writeBytes("Content-Disposition: form-data; name=\"Reason\"" + "\"" + crlf);
            request.writeBytes(crlf);
            request.writeBytes(URLEncoder.encode(getData[4], "UTF-8"));
            request.writeBytes(crlf);
    ...
    protected void onPostExecute(String s) {
        ...
        if (isSuccess != -1){
            ...
            Intent intent = new Intent(c,ReturnOnAttendanceMainActivity.class);
            ...
            intent.putExtra("s_level",getData[9]);
            intent.putExtra("classid",getData[10]);
            intent.putExtra("attendancedate",getData[3]);
